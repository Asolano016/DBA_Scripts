...
FROM #tInstanceRoles
WHERE LoginName NOT IN ('PRG-DC\SRV_PRGDC-QUALYS_PC','PRG-DC\U-DLG-PRGDC-SQL_Admins','PRG-DC\UDLDHL-SQLmigdist','sa')
...
SELECT InstanceName	
	  ...
	  ,IsDisabled
	  ,CASE 
			WHEN RoleName IN ('VIEW SERVER STATE','VIEW ANY DEFINITION')  THEN 'activity monitor'
			WHEN RoleName IN ('ALTER TRACE')  THEN 'alter trace'
			ELSE RoleName
	   END AS RoleName
FROM #tInstanceRoles
ORDER By LoginName, RoleName
