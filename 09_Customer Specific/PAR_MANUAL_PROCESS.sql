...
SELECT [name]
FROM sys.databases 
WHERE [name] NOT IN ('master','tempdb','model','msdb','Admin','SSISDB')
...
BEGIN
	IF NOT EXISTS (SELECT [name] FROM sys.server_principals WHERE [name] = @Login)
		SET @SQL = @LineBreak + "USE [master];" + "CREATE LOGIN [" + @Login + "] WITH PASSWORD='" + @SQLLoginPassword + "', DEFAULT_DATABASE=[master], CHECK_EXPIRATION=OFF;"  + @LineBreak
	ELSE
	BEGIN
		PRINT 'Login already exists.'
		SET @SQL = '--Starting Granting PAR Request--' + @LineBreak
